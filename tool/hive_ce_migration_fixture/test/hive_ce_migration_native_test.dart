...
void main() {
  ...
  test(
    ...
    () async {
      ...
      final sourceBox = await hive.Hive.openBox<dynamic>('legacy');
      ...
      expect(sourceBox.isOpen, isTrue);
      expect(sourceBox.get('string'), 'legacy');
      expect(sourceBox.get(7), 'integer-key');
  ...
  test(
    ...
    () async {
      final root =
          await Directory.systemTemp.createTemp('dxtr_hive_collision_');
      final hiveRoot = Directory('${root.path}/hive')..createSync();
      final dxtrRoot = Directory('${root.path}/dxtr')..createSync();
      addTearDown(() async {
        await hive.Hive.close();
      ...
      hive.Hive.init(hiveRoot.path);
      final sourceBox = await hive.Hive.openBox<dynamic>('collision');
      await sourceBox.put('@hive-int:1', 'string-key');
      await sourceBox.put(1, 'integer-key');
      ...
      expect(sourceBox.get('@hive-int:1'), 'string-key');
      expect(sourceBox.get(1), 'integer-key');
    },
    skip: nativeEnabled
        ? false
        : 'Set DXTR_BOX_NATIVE_TEST=1 to run Hive CE migration IO.',
  ...
  test(
    ...
    () async {
      final root =
          await Directory.systemTemp.createTemp('dxtr_hive_encrypted_');
      final hiveRoot = Directory('${root.path}/hive')..createSync();
      final dxtrRoot = Directory('${root.path}/dxtr')..createSync();
      addTearDown(() async {
        await hive.Hive.close();
      ...
      final sourceBox = await hive.Hive.openBox<dynamic>(
        'secure_legacy',
        encryptionCipher: hive.HiveAesCipher(hiveKey),
      );
      await sourceBox.put('token', <String, dynamic>{'value': 'secret'});
      ...
      expect(sourceBox.isOpen, isTrue);
      expect(sourceBox.get('token'), <String, dynamic>{'value': 'secret'});
    },
    skip: nativeEnabled
        ? false
        : 'Set DXTR_BOX_NATIVE_TEST=1 to run Hive CE migration IO.',
